...
function App() {
  ...
  useEffect(() => { load().catch(() => notify('Inbox data is temporarily unavailable.')); loadSession(); }, []);
  ...
  return (
    <div className="shell">
      ...
      <main className="main-area">
        ...
        {tab === 'inbox' && <InboxView decisions={decisions} stats={stats} question={question} setQuestion={setQuestion} askInbox={askInbox} ragBusy={ragBusy} answer={answer} undo={undo} approve={approve} correct={correct} />}
...
function InboxView({ decisions, stats, question, setQuestion, askInbox, ragBusy, answer, undo, approve, correct }: { decisions: Decision[]; stats: Stats; question: string; setQuestion: (v: string) => void; askInbox: () => void; ragBusy: boolean; answer: RagAnswer | null; undo: (id: string) => void; approve: (id: string) => void; correct: (d: Decision) => void }) {
